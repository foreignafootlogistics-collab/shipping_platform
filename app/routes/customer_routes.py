...
from app.utils.delivery_engine import calculate_real_distance
...
@login_required
def customer_message_restore(msg_id):
    m = DBMessage.query.get_or_404(msg_id)
...
@customer_bp.route("/messages/<int:msg_id>/unarchive", methods=["POST"])
@login_required
def customer_message_unarchive(msg_id):
    m = DBMessage.query.get_or_404(msg_id)
...
def view_message_attachment(attachment_id):
    ...
    filename = secure_filename(a.original_name or f"message_attachment_{a.id}")
...
@login_required
def schedule_delivery_detail(delivery_id):
    ...
    return render_template(
        "customer/schedule_delivery_detail.html",
        delivery=delivery,
        packages=packages
    )
...
@customer_bp.route(
    "/schedule-delivery/<int:delivery_id>/request-reschedule",
    methods=["POST"]
)
@login_required
def request_delivery_reschedule(delivery_id):
    ...
    if (delivery.status or "") not in allowed_statuses:
        return jsonify({
            "success": False,
            "message": (
    ...
    # ---------------------------------------
    # Cutoff:
    # 8 PM day before delivery
    # ---------------------------------------
    if delivery.scheduled_date:

        cutoff = datetime.combine(
            delivery.scheduled_date - timedelta(days=1),
            datetime.min.time()
        ).replace(
            hour=20,
            minute=0,
        ...
        if now_utc > cutoff:
            return jsonify({
                "success": False,
                "message": (
                    "Reschedule requests must be "
                    "submitted before 8 PM the "
                    "day before delivery."
                )
            }), 400

    new_date_raw = (
        request.form.get("requested_new_date")
        or ""
    ).strip()
    ...
    try:
        requested_new_date = datetime.strptime(
            new_date_raw,
            "%Y-%m-%d"
        ).date()
    except Exception:
    ...
    if requested_new_date <= date.today():
        return jsonify({
            "success": False,
            "message": (
                "Requested date must be "
                "a future date."
            )
        }), 400

    delivery.reschedule_requested = True
    delivery.reschedule_requested_at = (
        datetime.now(timezone.utc)
    )

    delivery.requested_new_date = (
        requested_new_date
    )

    delivery.reschedule_reason = reason
